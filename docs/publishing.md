...
`.github/workflows/ci.yml` verifies each push to `main`. After every verification job passes, its `publish` job builds the package and publishes with npm trusted publishing (OIDC).

- Pull requests never publish.
- Only the current `main` commit is eligible; older runs skip publication.
...
5. Push to `main` and inspect the Verify workflow's `publish` job. A commit already published manually is skipped; the next new commit exercises automatic publishing.
...
`main`에 푸시하면 전체 검사 성공 후 npm에 새 버전을 자동 게시합니다. 같은 버전은 덮어쓰지 않고 패치 번호를 증가시키며, 같은 커밋의 재실행은 건너뜁니다. 버전 변경을 저장소에 다시 커밋하지 않아 무한 배포가 발생하지 않습니다.

최초 게시에는 npm 로그인과 `@codestepteam` 게시 권한이 필요합니다. 최초 게시 후 npm의 신뢰 게시 설정에 `codestepteam/browser-assistant`, `ci.yml`, 직접 게시 허용을 등록합니다. 이후 GitHub Actions는 장기 토큰 없이 게시합니다.
