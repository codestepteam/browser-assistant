...
export const toolDescriptions: Record<ToolName, string> = {
  ...
  use_element:
    "마지막 화면에서 읽은 ref의 실제 UI 요소를 조작한다. click은 버튼·링크(value=null), fill은 입력창(value=문자열), select는 화면에 표시된 선택지의 value, check는 체크박스(value=boolean)다. expectedRevision과 고유 requestId가 필요하다. 결과에 최신 화면이 있으므로 다음 조작은 그 ref/revision을 사용한다. 저장·발송·삭제 같은 확정 버튼은 실행 내용을 표시하고 사용자 확인을 기다린 뒤 클릭한다. 확인은 사용자가 화면에서 직접 하며 도구 인자로 승인할 수 없다.",
  scroll_view:
